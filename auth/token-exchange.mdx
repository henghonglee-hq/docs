...
<Steps>
  ...
  <Step title="Create the customer">
    Call `POST /customers` with a unique `id` for the customer. Optionally include a `subject_token` (OIDC ID token) if you want to link the customer to an identity provider. GnosisRamp verifies the issuer, signature, allowed audiences, and skew settings you configured earlier.
  </Step>
...
<Note>
  The `subject_token` field is **optional**. If you don't use OIDC, you can create customers with just an `id` and optional `metadata`.
</Note>
...
| `400 Bad Request` | `id` missing or empty, or token validation failed. | Supply a valid `id` string in the request body. |
