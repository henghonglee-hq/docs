...
description: "Execute transactions and surface deposit instructions via intent sub-resources."
---
...
**Recommended polling interval:** every 5 seconds while the transaction is in `PENDING` or `IN_PROGRESS` status. Stop polling when the transaction reaches a terminal state (`SUCCESS`, `FAILED`, `CANCELLED`).

See the [Polling Best Practices](/intents/polling) guide for detailed recommendations.
...
Use outgoing webhooks (see the [Webhooks](/webhooks/outgoing) section) or poll the transaction endpoint to keep your UI up to date.
